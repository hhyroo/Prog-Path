  ...
  return (
    <div className="flex h-full flex-col gap-4">
      <div className="relative flex-1 overflow-hidden rounded-xl border-4 border-primary bg-card shadow-lg">
        {/* <Image src={maze.imageUrl || "/placeholder.svg"} alt={maze.name} fill className="object-contain p-4" priority /> */}
        <div>
          {maze.map}
        </div>
        {isRunning && (
        ...
        {isComplete && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/40">
            <div className="flex flex-col items-center gap-4 rounded-2xl bg-secondary p-8 shadow-2xl">
              ...
              <h3 className="text-2xl font-bold text-secondary-foreground">成功！</h3>
              <p className="text-secondary-foreground">迷路をクリアしました！</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="space-y-4 rounded-lg bg-card p-4 shadow-sm">
        <div>
          <h3 className="mb-2 text-lg font-bold text-foreground">{maze.name}</h3>
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={handleReset} variant="outline" className="gap-2 bg-transparent" size="lg">
        ...
        <div className="rounded-lg bg-accent/20 p-3">
          <p className="text-xs text-foreground">
            <strong>ヒント:</strong> カメラの前にARマーカーを配置して、プログラムを作成してください。
          </p>
        </div>
      </div>
    </div>
